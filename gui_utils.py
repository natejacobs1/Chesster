...
def convert_eval(score, is_black):
    """Convert engine score to a float value from black's perspective."""
    if score.is_mate():
        return float('inf') if (score.mate() > 0) != is_black else float('-inf')
    return score.white().score() / 100.0 * (-1 if is_black else 1)
...
from gpt_utils import askgpt, cost, schema
...
class ChessGUI(QWidget):
    def __init__(self, model_name, engine_name, engine_path, max_tokens=None, initial_url=None, initial_game_moves=None, max_history_length=None):
        super().__init__()
        self.setWindowTitle('Chesster: Analyze & Learn')
    ...
    def setup_chat_input(self, layout, font):
        ...
        self.chat_input.setPlaceholderText("Chat with Chesster")
    ...
    def on_input_enter(self):
        ...
        loading_text = "Chesster is thinking..."
        ...
        # Handle function call response (OpenAI shape). For Gemini, we fall back to using the
        # assistant content from chat history since Gemini responses don't have .choices.
        if hasattr(gpt_result, 'choices'):
            try:
                if gpt_result.choices[0].finish_reason == 'function_call':
                    ...
                    if fc.name == 'evaluate_move':
                        ...
                        if top_lines is None:
                            self.info_text.append(f"Chesster: The move {json.loads(fc.arguments)['move']} is not a legal move in this position")
                            print(f"total cost: {self.total_cost}$")
                            return
                        new_prompt = str(top_lines)
                        # Note: askgpt for Gemini doesn't support functions; this call will send the
                        # new prompt as a normal user message.
                        gpt_result = askgpt("", new_prompt, msgs=self.chat_msgs)
                        self.total_cost += cost(gpt_result, self.model_name)
            except Exception:
                # If anything about choices parsing fails, ignore function-call handling.
                pass

        # Print the response with HTML formatting
        # Prefer the chat history assistant content (works for Gemini and OpenAI-like wrappers).
        assistant_content = None
        try:
            assistant_content = chat_history[-1].get('content') if isinstance(chat_history, list) and len(chat_history) > 0 else None
        except Exception:
            assistant_content = None

        if not assistant_content and hasattr(gpt_result, 'choices'):
            try:
                assistant_content = gpt_result.choices[0].message.content
            except Exception:
                assistant_content = str(gpt_result)

        assistant_content = assistant_content or "(no content)"
        gpt_text = f"Chesster: {assistant_content}"
        gpt_text_html = re.sub(r"\*\*(.*?)\*\*", r"<b>\1</b>", gpt_text.replace('\n', '<br>'))
        gpt_text_html = gpt_text_html.replace("Chesster:", '<span style="color: blue;">Chesster:</span>', 1)
